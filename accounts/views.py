...
def login_view(request):
    email = request.data.get(
        "email"
    )  # get the email and password from frontend request body
    ...
    )  # verifies the password & username using django built-in authentication system
    if user is None:
        return Response({"error": "Invalid password"}, status=401)
    ...
    if user.role == "client":  # check if the logged user is client
        if (
            user.client is None or user.client.status != "approved"
        ):  # check the client approvel status
    ...
    return Response(
        {
            ...
            "user": {
                ...
                "role": user.role,
                "client_id": (
                    user.client.client_id if user.client else None
                ),  # If user linked with client: return client id otherwise return None
...
def delete_team_member(request, id):
    ...
    return Response(
        {"message": "Team member deleted successfully"}, status=status.HTTP_200_OK
    )
